...
def classify(symbol_cnt: List[np.ndarray], mask, cls_order, cls_weight, scale=255):
    ...
    cls_model.load_state_dict(
        torch.load(cls_weight, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu")))
...
class Net(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.fc1 = nn.Linear(2304, 256)  # First channel size varies when you change parameter
...
def predict(model, image: np.ndarray, targets: list,
...
def parse_args():
    ...
    parser.add_argument('--train_path', default='images/symbol/train', help='the dir to training images')
    parser.add_argument('--test_path', default='images/symbol/test', help='the dir to testing images')
...
def main(image_type, debug=False):
    ...
    train_transforms = A.Compose([
        A.SafeRotate(limit=[-180, 180], border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=1, p=1),
    ...
    if debug:
        ...
        plt.show()
        visualize_augmentations(model, test_dataset, idx_to_class, np.random.randint(1, len(test_image_paths)),
                                random_img=True)
...
def visualize_augmentations(model, dataset, idx_to_class, samples=10, cols=5, random_img=False):
    import matplotlib.pyplot as plt
    import copy
