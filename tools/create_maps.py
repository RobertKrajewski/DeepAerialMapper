...
def create_map_from_semantic_mask(
    ...
    lanemarkings = derive_lanemarkings(
...
def derive_lanemarkings(
    ...
    road_borders = [
        Lanemarking([c], Lanemarking.LanemarkingType.ROAD_BORDER) for c in road_borders
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        default="data/seg_masks/",
        help="location of input segmentation folder.",
    )
    parser.add_argument(
        "--output",
        default="results/maps/<now>",
        help="Location of the root output directory.",
    )
    parser.add_argument(
        "--start_map", default=0, help="Index of the first image to process"
    )
    ...
    for i_segmentation_file, segmentation_file in enumerate(segmentation_files):
        if i_segmentation_file < start_map:
            logger.info(
                f"Skipping map {i_segmentation_file} as start_map is set to {start_map}!"
            )
            continue
        logger.info(
