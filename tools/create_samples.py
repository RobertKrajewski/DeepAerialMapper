...
def create_straight_road_borders(add_lanemarkings=False):
    ...
    vegetation_color = np.asarray(palette_map[SemanticClass.VEGETATION], np.uint8).reshape((1, 1, 3))
    road_color = np.asarray(palette_map[SemanticClass.ROAD], np.uint8).reshape((1, 1, 3))
    lanemarking_color = np.asarray(palette_map[SemanticClass.LANEMARKING], np.uint8).reshape((1, 1, 3))
    ...
    img = draw_horizontal_road(img, road_color, 60, 60, lanemarking_color, lanemarking_width)

    # Add double lane road (6m == 120px)
    img = draw_horizontal_road(img, road_color, 180, 120, lanemarking_color, lanemarking_width)
    img = draw_horizontal_lanemarking(img, 240, lanemarking_width, lanemarking_color)

    # Add triple lane road (9m == 180px)
    img = draw_horizontal_road(img, road_color, 360, 180, lanemarking_color, lanemarking_width)
...
def draw_horizontal_road(img, road_color, y_start, width, lanemarking_color, outer_lanemarking_width=0):
    s = y_start
    e = y_start + width
...
def draw_horizontal_lanemarking(img, y_center, width, color, segment_length=None, gap_length=0):
    if width == 0:
        return img
...
def crate_dashed_lanemarkings():
    ...
    lanemarking_width = 2
    s = 60
    img = draw_horizontal_road(img, road_color, s, 180, lanemarking_color, lanemarking_width)
    img = draw_horizontal_lanemarking(img, s+60, lanemarking_width, lanemarking_color)
    img = draw_horizontal_lanemarking(img, s+120, lanemarking_width, lanemarking_color)

    lanemarking_width = 2
    s += 210
    img = draw_horizontal_road(img, road_color, s, 180, lanemarking_color, lanemarking_width)
    img = draw_horizontal_lanemarking(img, s+60, lanemarking_width, lanemarking_color, segment_length=60, gap_length=100)
    img = draw_horizontal_lanemarking(img, s+120, lanemarking_width, lanemarking_color, segment_length=60, gap_length=100)

    lanemarking_width = 4
    s += 210
    img = draw_horizontal_road(img, road_color, s, 180, lanemarking_color, lanemarking_width)
    img = draw_horizontal_lanemarking(img, s+60, lanemarking_width, lanemarking_color, segment_length=60, gap_length=40)
    img = draw_horizontal_lanemarking(img, s+120, lanemarking_width, lanemarking_color, segment_length=60, gap_length=40)
